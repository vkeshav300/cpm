...
{
  /**
   * @brief Initializes CPM in a directory, and creates all the files needed for a
   * basic project.
   *
  ...
  {
    std::vector<std::string> default_folders = {"src", "include", "build",
                                                "tests", "docs"};
    std::vector<std::string> default_files = {".gitignore", "CMakeLists.txt",
                                              "README.md", "LICENSE", ".cpm"};
    ...
    std::string file_main_path = "src/" + project_name + "." + language;
    default_files.emplace_back(file_main_path);
...

    {
      ...
      file_cmake << "# Minimum required version of CMake (cmake --version)\n"
                 ...
                 << "    # Specify the installation directory for macOS (e.g., "
                    "/usr/local/bin)\n"
    ...
    {
      std::ofstream file_main(file_main_path);
    ...
    }
    {
      ...
      file_cpm << "language: " << language;
      file_cpm.close();
    }

    logger::success("populated files");

    logger::custom("to best utilize this project structure, it is recommended "
                   "that you know how to use CMake.",
                   "important", "yellow");

    if ("PLACEHOLDER" == project_name)
      logger::custom("make sure to edit all the placeholders in your "
                     "\"CMakeLists.txt\" file.",
                     "important", "yellow");
  ...
  int file_pair(const std::vector<std::string> &arguments, const bool hpp, const std::string language)
  {
    ...
    const std::vector<std::string> pair_names =
        misc::sub_vector(arguments, 1, arguments.size());
    ...
    if ("new" == sub_command)
    {
      for (auto &pair_name : pair_names)
      {
        ...
        {
          std::ofstream file_header("./include/" + pair_name +
                                    header_file_extention);
  ...
  int help()
  {
    ...
    std::cout
        ...
        << "--version || version --> tells current version of cpm you are "
           "using\n\n"
        << "init <language> --> sets up a new C or C++ project\n"
        << "init flag : -post --> sets up CPM in a preexisting project\n"
        << "init flag : -v=<version> --> specifies custom version of programming "
           "language to be used for compiling\n"
        << "pair new <name> --> creates header/source file pair\n"
        << "pair remove <name> --> gets rid of header/source file pair\n"
        << "pair flag : -hpp --> (if applicable to the sub-command), command "
           "will use .hpp files instead of .h files\n\n"
        << "contents copy <copy from> <copy to> --> copies contents of one file "
           "to another (will erase all data from copy to file)\n"
        << "contents erase <file> --> erases all contents from a file\n"
        << "contents switch <file 1> <file 2> --> switches the contents of the "
           "two files\n"
        << "contents flag : -app/-append --> (if applicable to the sub-command), "
           "will use append mode\n\n"
  ...
  int contents(const std::vector<std::string> &arguments, const std::vector<std::string> &flags)
  {
    ...
    if ("copy" == sub_command)
    {
...

      {
        ...
        if (misc::find_in_vector(flags, "append") ||
            misc::find_in_vector(flags, "app"))
      ...
      logger::success((misc::find_in_vector(flags, "append") ||
                       misc::find_in_vector(flags, "app"))
                          ? "copied " + file_primary_s + " to " + file_target_s +
                                " with append mode"
                          : "copied " + file_primary_s + " to " + file_target_s +
                                " without append mode");
    }
    ...
    else if ("switch" == sub_command)
    {
      ...
      std::filesystem::rename((file_primary_s + ".tmp").c_str(),
                              file_primary_s.c_str());
      std::filesystem::rename((file_target_s + ".tmp").c_str(),
                              file_target_s.c_str());

      logger::success("switched contents of " + file_primary_s + " and " +
                      file_target_s);
    }
    else
    {
  ...
  int install(const std::vector<std::string> &arguments, const std::vector<std::string> &flags, const std::string language)
  {
    ...
    std::string target_url =
        "https://github.com/" + arguments[0] + "/releases/latest";
    ...
    // release_info[0]["assets"][0]["browser_download_url"].asString().c_str());

    // std::ofstream output_file("cpm_install.tmp.zip", std::ios::binary);
    // curl_easy_setopt(curl.get(), CURLOPT_WRITEFUNCTION,
    // misc::write_file_callback); curl_easy_setopt(curl.get(), CURLOPT_WRITEDATA,
    // &output_file);

    // // TODO https://api.github.com/repos/{argument[0]}/releases -- validate api
    // and download data from API
    ...
    // if (res != CURLE_OK)
    // {
    //     logger::error_q(": curl_easy_perform() : asset download failed",
    //     curl_easy_strerror(res)); logger::custom("try seeing if the latest
    //     release on the repository contains a 'cpm_install.zip' file", "hint",
    //     "yellow"); return 1;
    // }
   ...
   * @return int
   */
  int uninstall(const std::vector<std::string> &arguments) { return 0; }
} // namespace commands
